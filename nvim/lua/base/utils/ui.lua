...
--  to keep it as it takes a lot of complexity out of `../keymaps.lua`.
...
function M.toggle_autoformat()
  ...
  utils.notify(string.format("Global autoformatting %s", bool2str(g.autoformat_enabled)))
end
...
function M.toggle_background()
  ...
  utils.notify(string.format("background=%s", go.background))
end
...
function M.toggle_buffer_autoformat(bufnr)
  ...
  utils.notify(string.format("Buffer autoformatting %s", bool2str(b[bufnr].autoformat_enabled)))
end
...
function M.toggle_buffer_inlay_hints(bufnr)
  ...
  utils.notify(string.format("Buffer inlay hints %s", bool2str(b[bufnr].inlay_hints_enabled)))
end
...
function M.toggle_buffer_syntax(bufnr)
  ...
  utils.notify(string.format("syntax %s", bool2str(bo[bufnr].syntax)))
end
...
function M.toggle_codelens(bufnr)
  ...
  utils.notify(string.format("CodeLens %s", bool2str(g.codelens_enabled)))
end
...
function M.toggle_conceal()
  ...
  utils.notify(string.format("conceal %s", bool2str(opt.conceallevel:get() == 2)))
end

--- Toggle diagnostics
function M.toggle_diagnostics()
  g.diagnostics_mode = (g.diagnostics_mode - 1) % 4
  _vim.diagnostic.config(require("base.utils.lsp").diagnostics[g.diagnostics_mode])
...
function M.toggle_foldcolumn()
  ...
  utils.notify(string.format("foldcolumn=%s", wo.foldcolumn))
end
...
function M.toggle_inlay_hints(bufnr)
  ...
  utils.notify(string.format("Global inlay hints %s", bool2str(g.inlay_hints_enabled)))
end
...
function M.toggle_paste()
  ...
  utils.notify(string.format("paste %s", bool2str(opt.paste:get())))
end
...
function M.toggle_signcolumn()
  ...
  utils.notify(string.format("signcolumn=%s", wo.signcolumn))
end

--- Toggle spell
function M.toggle_spell()
  wo.spell = not wo.spell -- local to window
  utils.notify(string.format("spell %s", bool2str(wo.spell)))
end
...
function M.toggle_tabline()
  ...
  utils.notify(string.format("tabline %s", bool2str(opt.showtabline:get() == 2)))
end

--- Toggle notifications for UI toggles
function M.toggle_ui_notifications()
  g.notifications_enabled = not g.notifications_enabled
  utils.notify(string.format("Notifications %s", bool2str(g.notifications_enabled)))
end
...
function M.toggle_url_effect()
  ...
  utils.notify(string.format("URL effect %s", bool2str(g.url_effect_enabled)))
end

--- Toggle wrap
function M.toggle_wrap()
  wo.wrap = not wo.wrap -- local to window
  utils.notify(string.format("wrap %s", bool2str(wo.wrap)))
end
...
function M.toggle_zen_mode(bufnr)
  ...
  utils.notify(string.format("zen mode %s", bool2str(b[bufnr].zen_mode)))
  cmd "ZenMode"
end
